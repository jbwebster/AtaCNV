...
#' @param K number of clusters. Only used when \code{cell_cluster} is set to "none".
...
normalize <- function(count, genome="hg38",
  ...
  if(!identical(normal_cells, "none")){
    normal_cells <- intersect(colnames(count)[f2], colnames(count[,normal_cells]))
  }
